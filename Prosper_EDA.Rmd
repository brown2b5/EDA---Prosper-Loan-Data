...
The majority of borrowers in the Completed and Current loan status categories made greater than $50,000 dollars.  The majority of borrowers in the Defaulted and Chargedoff categories made less than $50,000 dollars.  Both these results are not surprising as having more income will make it easier to pay off debt.
...
For the categorical variables, the strongest relationship I found was for loan status filled by income range bar chart.  For each of the best case scenarios (Completed and Current), borrowers making greater than $50,000 dollars had greater proportions.  For each of the worst case scenarios (Defaulted and Chargedoff), borrowers making less than $50,000 dollars had greater proportions.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 9, fig.width = 10,debt_to_income_ratio_vs_credit_score_income_range_loan_status}
...
From the faceted scatter plots shown (note that each color in the legend represents 10 data points), it appears that there are far less borrowers with an income greater than $50,000 in the Chargedoff and Defaulted categories than in the Completed and Current categories.  It also looks as if there is around an equal number of borrowers both below and above $50,000 for the Defaulted and Chargedoff categories whereas for the Completed and Current categories, there are more borrowers with over $50,000 dollars in income. These results should be expected as having more income will make it easier to pay back loans. There are far more borrowers with a lower range of credit score above 800 for the Current and Completed loan status than for the Defaulted and Chargedoff loan status.  For the Chargedoff and Defaulted categories, there seems to be more borrowers with a lower range of credit score below 700 than above 700. These results too should be expected as borrowers with a lower credit score will have more risk of Defaulting or being Chargedoff.  Debt to income ratio doesn't appear to play much of a part in determining loan status.  In each of the loan status categories, there appears to be more borrowers below a ratio of 0.5 than above.  In summary, it does appear that having a low credit score and income range makes it more likely that a borrower will end up Defaulting or being charged off with a loan (the probabilities for both, regardless of income/credit score may still be less than 50% as there are more borrowers in general who fall into the Current and Completed status).  It doesn't appear however that debt to income ratio plays a large part in determining loan status.  This is supported by what I found in the univariate case as I thought credit score range lower had a much stronger relationship to loan status than debt to income ratio.  

Now I will look at borrower apr versus credit score range lower scatterplots faceted by loan status and filled by income range.  Debt to income ratio didn't have a big affect on loan status but I do think borrower apr will based off the bivariate analysis.  This will be a very similar visual as the one previously.

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 9, fig.width = 10,borrower_apr_vs_credit_score_income_range_loan_status}
...
From the faceted scatter plots shown (note that each color in the legend represents 10 data points), it appears first of all that there are many more borrowers falling into the Completed and Current status rather than the Defaulted or Chargedoff status.  There is a great deal more borrowers with incomes over $50,000 for both the Completed and Current loan status than for the Defaulted and Chargedoff loan status.   Just like the debt to income ratio versus credit score scatter plot, there appears to be many more borrowers with credit scores beyond 800 for the Current and Completed status than for the Defaulted and Chargedoff loan status.  There also appears to be more borrowers to the left of 700 for both the Defaulted and Chargedof loan status than for the Completed and Current loan status.  In terms of borrower apr, it looks like there are more borrowers below 0.2 than above 0.2 for both Completed and Current loan status while the opposite is true for both Defaulted and Chargedoff loan status (actually from the box plot, there are more borrowers above 0.2 for the current status but there are for more in this range for Defaulted and Chargedoff).  From these scatterplots, it does appear that having a higher income, a lower apr and a high credit score are all indicators that the probability of Defaulting or being Chargedoff on a loan will be minimized. 
