...
the original loan amount and interest. I expect to see which borrower employment status, credit score, debt to income ratio and other loan variables are most associated with those who default on their loans and who successfully complete their loan payments.

```{r echo=FALSE, include=FALSE, message=FALSE, warning=FALSE, packages}
# I added include=FALSE so that messages indicating the packages were loaded wouldn't appear in the knitted HTML file (Reference 51).

# Loading all of the packages I will use in this project. I will be using 
# ggplot2 to construct plots (Reference 1), knitr to put the R markdown document into an HTML document (Reference 2) and dplyr to mutate and transform the data (Reference 3).  I had to add a repos section to get the knitting of the html file to complete (Reference 4).  
...
# Install lemon package to work with legends using ggplot (Reference 42)

install.packages("lemon", dependencies = T, 
                 repos = "http://cran.us.r-project.org")

# Install RColorBrewer so that I can have a better color scheme in my graphs
# (Reference 43)
...
setwd(paste("C:/Users/brownb/Desktop/Udacity/Data Analyst Nanodegree/",
            "Term 2/Exploratory Data Analysis/EDA - Prosper Loan Data", 
            sep = ""))
```
...
variable is related to measures such as the number of borrowers who default and the number who successfully pay back their loans.  Once we have the univariate plots, we can have an idea about what variables we may want to explore in a multi-variate fashion.
...
From this plot, the category with the highest proportion of borrowers is clearly the Current loans.  I lumped all the Past Due categories into one Past Due category since I only really care about Completed, Defaulted and Chargedoff
...
ggplot(subset(prosp_sub, income_verifiable == 'True' &
                !is.na(income_range)), aes(income_range)) +
                geom_bar(aes(y=..count../sum(..count..))) + 
                ylab('Proportion of Borrowers')
...
# Get summary of employment status duration while excluding NA's.

summary(subset(prosp_sub, employment_status %in% condition &
                 !is.na(employment_status_duration))$employment_status_duration)
```
...
outliers for those not employed of multiple years.  I cannot see Prosper letting a borrower have a loan who had not been employed for many years (I originally let the employment status duration for those not employed to be negative).  There are still outliers in the data here.  From the summary, the maximum number of months worked is 745 or around 63 years.  This may actually be possible, but if I graph this data against loan status or any other variable
...
head(subset(prosp_sub, employment_status %in% condition &
              !is.na(employment_status_duration) &
              employment_status_duration > 400))
```
...
# while excluding NA values.  


q1 <- ggplot(subset(prosp_sub, !is.na(credit_score_range_lower)),
             aes(credit_score_range_lower)) + geom_histogram() 
...
q2 <- ggplot(subset(prosp_sub, !is.na(credit_score_range_lower)),
             aes(credit_score_range_lower)) + geom_histogram() +
             coord_cartesian(xlim = c(375,880))

# Plot histogram while zooming in to the range of most of the data and picking a better binwidth (Reference 20)

q3 <- ggplot(subset(prosp_sub, !is.na(credit_score_range_lower)),
             aes(credit_score_range_lower)) + geom_histogram(binwidth = 1) +
             coord_cartesian(xlim = c(375,880))
...
# Get summary of the low range of credit score while excluding NA values. 

summary(subset(prosp_sub,!is.na(credit_score_range_lower))$
          credit_score_range_lower)
```

From the three plots of the histograms, each more finely tuned than the next, 
I notice a few things.  The first is that there are some borrowers with a credit score of zero.  I am going to see what the rest of the variables looks like for some of these borrowers to see if I should discard them or if I should
...
to disappear from the data. Another thing I noticed after changing the binwidth is that there appears to be only integer values of credit score in increments of 20. There do not appear to be values that aren't whole or that fall between 
...
head(subset(prosp_sub, !is.na(credit_score_range_lower) &
            credit_score_range_lower == 0))
```
...
# Plot the initial distribution of debt to income ratios (Reference 20) while excluding NA's.


q1 <- ggplot(subset(prosp_sub,!is.na(debt_to_income_ratio)),
             aes(debt_to_income_ratio)) + geom_histogram() 
...
q2 <- ggplot(subset(prosp_sub,!is.na(debt_to_income_ratio)),
             aes(debt_to_income_ratio)) + geom_histogram(binwidth = 0.1) +
             coord_cartesian(xlim = c(0,1))

# Plot histogram while zooming in to the range of most of the data and picking a better binwidth (Reference 20)

q3 <- ggplot(subset(prosp_sub,!is.na(debt_to_income_ratio)),
             aes(debt_to_income_ratio)) + geom_histogram(binwidth = 0.005) +
             coord_cartesian(xlim = c(0,1))
...
# Plot the initial distribution of borrower apr (Reference 20) while excluding NA's


q1 <- ggplot(subset(prosp_sub,!is.na(borrower_apr)), aes(borrower_apr)) + 
             geom_histogram() 
...
q2 <- ggplot(subset(prosp_sub,!is.na(borrower_apr)), aes(borrower_apr)) + 
        geom_histogram() + coord_cartesian(xlim = c(0,0.4))

# Plot histogram while zooming in to the range of most of the data and picking a better binwidth (Reference 20)

q3 <- ggplot(subset(prosp_sub,!is.na(borrower_apr)), aes(borrower_apr)) + 
...
summary(subset(prosp_sub,!is.na(borrower_apr))$borrower_apr)
```
...
can see that there is minimal difference between the conditions.  The proportion of borrowers who own a home is slightly greater than 50%.  It's hard to determine how well this variable predicts loan payback success rate without
using bi-variate and multi-variate graphic capabilities in R.  I will facet loan status over each of these conditions to see which condition has a higher level of success and failure.  It would make sense that borrowers owning a home will be more likely to payback their loans than those that do not own a home. 
...
proportion of borrowers is that with a rating of C.  As with the prosper scores, this is reasonable since this is the average prosper rating.  Most borrowers will not have excellent or bad credit but have average credit.  The two ratings with the lowest and second lowest proportion of borrowers are the AA and HR ratings, respectively.  As I mentioned with the prosper score, I will need to use R's bi-variate graphing capabilities to determine the relationship between prosper score and loan status. 
...
# Create the bar graph of proportion of borrowers for best possible combination (References 17, 18 and 41)
...
# Get summary of the data for the best and worst combo
summary(subset(prosp_2009, best_combo)$loan_status)
summary(subset(prosp_2009, worst_combo)$loan_status)
```

The first plot in the visual directly above is the loan status for the best 
possible combination of prosper rating and score (AA and 9-11) while the second plot is the loan status for the worst possible combination of prosper rating and score (HR and 1-3). Clearly there is a higher precedence of loans being past due, defaulted and charged off for the worst combination than the best 
combination.  There is also a higher percentage of loans which are completed in the best combination versus the worst combination.  This would seem to point to a relationship between these three variables.  I will be examining this further in the bi-variate and multi-variate portion of my analysis.  
...
# combinations.  I pick a max of 125 since all the data is below this number (References 18, 19 and 41).  I used coord_cartesian so that I wouldn't shave off any data.
...
best (AA and 9-11) and worst (HR and 1-3) possible combination of prosper scores and ratings as I did with the loan status variable.  As I did in the original plots of days' delinquent, I limited the data to cases where the days' delinquent was greater than 0 and to cases where the loan had not been charged 
...
This concludes the univariate portion of the analysis.  The following are some
summaries about what I have observed so far.
...
associated with higher success and failure rates for paying back the loans.  If a borrower owns a home, that means a bank has already approved them for a loan
...
categories for this variable.  I combined these categories to lessen the clutter of the bar chart. 
...
For all the variables, wherever NA's were present, I filtered them out of the
data.  
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10, Bivariate_Plots_GGally_categorical_loan_status}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10, GGally_categorical_loan_status_prosper}
...
bi-variate analysis.  A prosper score of 1-3, 5 and 6 has highest proportions of borrowers with a loan status of chargedoff.  

A surprising result was that a prosper rating of D had the highest proportion
of borrowers with a completed loan.  The three worst prosper ratings (HR, E, and D) had the highest proportion of borrowers who had been chargedoff.  As with prosper score, prosper ratings effect on borrowers who had defaulted was difficult to see.

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 9, fig.width = 10, debt_to_income_ratio_vs_credit_score}
...
q1 <- ggplot(aes(x = credit_score_range_lower, y = debt_to_income_ratio), 
             data = subset(prosp_sub, !is.na(credit_score_range_lower) &
                             !is.na(debt_to_income_ratio))) + geom_point()
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10, credit_score_vs_income_range}
...
# and for which the income range and credit score is not NA (References 38 and 39)

ggplot(subset(prosp_sub, income_verifiable == 'True' & !is.na(income_range) &
                !is.na(credit_score_range_lower)), 
...
# (only considering income that is verifiable and without NA's).

prosp_income <- subset(prosp_sub, income_verifiable == 'True' &
                !is.na(income_range)) 

# Get summaries of credit_score_range_lower grouped by income_range
# (Reference 40) excluding NA's for credit score.

by(subset(prosp_income, !is.na(credit_score_range_lower))
   $credit_score_range_lower, subset(prosp_income,
   !is.na(credit_score_range_lower))$income_range, summary)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10,employment_duration_vs_loan_status}
...
# Find box plot for the given employment condition and excluding NA's for employment duration.  I also zoomed in on the y axis to better see the medians and where they are (References 38 and 39).
...
# (Reference 40) excluding NA's

by(subset(prosp_edur,
          !is.na(employment_status_duration))$employment_status_duration, subset(prosp_edur,
          !is.na(employment_status_duration))$loan_status, summary)
```
...
at or greater than $100,000 and those with income range of $1-24,999 is
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10,credit_score_vs_loan_status}

# This chunk creates a box plot of credit score lower range versus loan status

# Find box plot for non-NA credit scores. I also zoomed in on the y axis to better see the medians and where they are (References 38 and 39).

ggplot(subset(prosp_sub, !is.na(credit_score_range_lower)), 
       aes(x = loan_status, y = credit_score_range_lower)) +
                  geom_boxplot() + 
                  stat_summary(fun.y=mean, geom="point", color = "red",
                               shape = 4, size=3) + 
...
# (Reference 40)
by(subset(prosp_sub, !is.na(credit_score_range_lower))$credit_score_range_lower,
   subset(prosp_sub, !is.na(credit_score_range_lower))$loan_status, summary)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10,debt_to_income_ratio_vs_loan_status}

# This chunk creates a box plot of debt to income ratio versus loan status

# Find box plot for non-NA debt to income ratio. I also zoomed in on the y axis to better see the medians and where they are (References 38 and 39).

ggplot(subset(prosp_sub, !is.na(debt_to_income_ratio)), 
       aes(x = loan_status, y = debt_to_income_ratio)) + geom_boxplot() + 
                  stat_summary(fun.y=mean, geom="point", color = "red",
                               shape = 4, size=3) +
...
# (Reference 40) excluding NA's.
by(subset(prosp_sub, !is.na(debt_to_income_ratio))$debt_to_income_ratio, subset(prosp_sub, !is.na(debt_to_income_ratio))$loan_status, summary)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10,borrower_apr_vs_loan_status}

# This chunk creates a box plot of borrower apr versus loan status

# Find box plot for non-NA borrower apr's. I also zoomed in on the y axis to better see the medians and
# where they are (References 38 and 39).

ggplot(subset(prosp_sub, !is.na(borrower_apr)), 
       aes(x = loan_status, y = borrower_apr)) + geom_boxplot() + 
                  stat_summary(fun.y=mean, geom="point", color = "red",
                               shape = 4, size=3) +
                  coord_cartesian(ylim = c(0,0.5))
...
# (Reference 40) excluding NA's.
by(subset(prosp_sub, !is.na(borrower_apr))$borrower_apr, 
   subset(prosp_sub, !is.na(borrower_apr))$loan_status, summary)
```
...
largest mean and median borrower apr but several of these borrowers may be close to defaulting.  And they should have a higher borrower apr on average than those who are current or completed.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10, loan_status_employment_status_graph}
...
# First I will get rid of the factor levels I do not use in prosp_edur for
# employment status (Reference 15)

prosp_edur$employment_status <- factor(prosp_edur$employment_status, 
                    levels = c("Employed", "Full-time", "Not employed",
                               "Part-time", "Self-employed"))

# I am using scale_fill_brewer to get a better color scheme (Reference 43)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10, loan_status_income_range_graph}
# Plotting a bar graph of loan_status; proportions for each category filled
...
# (Reference 40)
by(prosp_income$income_range, prosp_income$loan_status, 
   summary)
```
...
for loan statuses of Defaulted or Chargedoff.  Income ranges of $0-49,999 also
make up a greater percentage of all borrowers for Chargedoff and Defaulted loans than for Current and Completed loans.  This should all be expected as having more money will make it easier to pay back loans.  It has been shown that income range plays a large part in the ability of borrowers to pay back loans with more income being associated with a better ability to pay back loans (Reference 12) so this is not surprising.  Note that I am only considering income that is verifiable and has been reported.

Now I will see the affect being a homeowner has on loan status by filling the
loan status bar charts with is_homeowner proportions.

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10, loan_status_is_homeowner_graph}
# Plotting a bar graph of loan_status; proportions for each category filled
# by the proportion of each bar under each is_homeowner category (References 17 and 18).

# I am using scale_fill_brewer to get a better color scheme (Reference 43)

ggplot(prosp_sub, aes(loan_status, fill = is_homeowner)) + 
  geom_bar(aes(y=..count../sum(..count..))) + ylab('Proportion of Borrowers') +
  scale_fill_brewer(palette = "BrBG") 

# Get summaries of loan status grouped by whether the borrower is a homeowner
# (Reference 40)
by(prosp_sub$is_homeowner, prosp_sub$loan_status, 
   summary)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10, loan_status_prosper_score_graph}
# Plotting a bar graph of loan_status; proportions for each category filled
# by the proportion of each bar under each prosper score category (References 17 and 18).  I am using the dataframe for data post July 2009.

# I am using scale_fill_brewer to get a better color scheme (Reference 43)

ggplot(prosp_2009, aes(loan_status, fill = prosper_score)) + 
  geom_bar(aes(y=..count../sum(..count..))) + ylab('Proportion of Borrowers') +
  scale_fill_brewer(palette = "BrBG") 

# Get summaries of loan status grouped by prosper score
# (Reference 40)
by(prosp_2009$prosper_score, prosp_2009$loan_status, 
   summary)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10, loan_status_prosper_rating_graph}
# Plotting a bar graph of loan_status; proportions for each category filled
# by the proportion of each bar under each prosper rating category (References 17 and 18).  I am using the dataframe for data post July 2009.

# I am using scale_fill_brewer to get a better color scheme (Reference 43)

ggplot(prosp_2009, aes(loan_status, fill = prosper_rating)) + 
  geom_bar(aes(y=..count../sum(..count..))) + ylab('Proportion of Borrowers') +
  scale_fill_brewer(palette = "BrBG") 

# Get summaries of loan status grouped by prosper rating
# (Reference 40)
by(prosp_2009$prosper_rating, prosp_2009$loan_status, 
   summary)
```
...
The median employment duration increased from a loan status of Completed to 
Current but then decreased from Current to Chargedoff.  There was minimal
difference (1 month) between mean employment duration for Completed and Defaulted loans and a difference of 2 months between Completed and Chargedoff loans.  The difference was also around 2 months for the means.  Because of the minimal difference in employee duration, I will not consider this variable in the multi-variate analysis.  
...
For prosper score, more loans with a status of Complete fall into a prosper 
score of 7-11 than a prosper score of 1-5.  For statuses of Defaulted and Chargedoff, more loans fall into a prosper score of 1-5 than 7-11.  This is to be expected since a low prosper score (1,2,3, etc...) is associated with higher risk of Defaulting and being charged off.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 9, fig.width = 10,Multivariate_Plots_debt_to_income_ratio_vs_credit_score_income_range}

# In this chunk I examine the relationship between debt_to_income_ratio and
# lower range of credit score filled by income range.  I do this first by finding # an initial, un-changed scatterplot of debt_to_income_ratio versus lower range # of credit score filled by income range.  I then deal with overplotting issues and outliers while still filling the data with income range.  I do this to see which portion of the graph from the bivariate case is associated most with each income range.  

# Plot initial form of scatterplot for debt_to_income_ratio versus 
# credit_score_range_lower filled by income range (Reference 30) while excluding # NA's.  Note that I am using the prosp_income dataset because I only want 
# income that is verfiable and that does not contain NA's.  I am using 
# scale_color_brewer to have a better color scheme (Reference 43).

q1 <- ggplot(aes(x = credit_score_range_lower, y = debt_to_income_ratio, 
                 color = income_range), 
             data = subset(prosp_income, !is.na(credit_score_range_lower) &
                             !is.na(debt_to_income_ratio))) + geom_point() +
             scale_color_brewer(palette = "BrBG")

# Set values which will be limits for scatterplot without data points at 
# debt to income ratio of 10 using quantile (Reference 31).
y_lim = c(0,quantile(prosp_sub$debt_to_income_ratio,0.99, na.rm='True'))

# Graph scatter plot where the data is jittered and with a transparency of
# 1/20 to correct for overplotting (Reference 32).  I also cut off the data at debt to income ratio of 10 and credit score range lower of 0 (Reference 21). They are outliers even though the data is good.  I set position_jitter with an h of 0 to keep the data in the vertical direction from becoming negative (Reference 33).  I am using scale_color_brewer to have a better color scheme (Reference 43).
...
q2 <- ggplot(aes(x = credit_score_range_lower, y = debt_to_income_ratio, 
 ...
 # Plot the graphs on the same grid (Reference 44)
 grid_arrange_shared_legend(q1, q2, ncol = 1, nrow = 2, position = 'right')
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 9, fig.width = 11,debt_to_income_ratio_vs_credit_score_income_range_loan_status}

# In this chunk I examine the relationship between debt_to_income_ratio and
# lower range of credit score filled by income range and faceted over loan status.  I will only consider the second type of plot from the previous R chunk (with a transparency and jitter while cutting off outliers).  

# Set values which will be limits for scatterplot without data points at 
# debt to income ratio of 10 using quantile (Reference 31).
y_lim = c(0,quantile(prosp_sub$debt_to_income_ratio,0.99, na.rm='True'))

# Construct similar scatter plots to the transparent, jittered scatter plot in the previous R chunk but facet it over loan status (Reference 45). I had to add a guides command so that I could change the transparency of the colors in the legend (Reference 46). Note again that I am only considering incomes with the condition that they are verifiable and are not NA's (prosp_income).

ggplot(aes(x = credit_score_range_lower, y = debt_to_income_ratio, 
           color = income_range), 
       data = subset(prosp_income, !is.na(credit_score_range_lower) &
                             !is.na(debt_to_income_ratio))) + 
        geom_jitter(alpha = 1/10, position = position_jitter(h = 0)) + 
        coord_cartesian(ylim = y_lim) + scale_color_brewer(palette = "BrBG") +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 9, fig.width = 11,borrower_apr_vs_credit_score_income_range_loan_status}

# In this chunk I examine the relationship between borrower_apr and
# lower range of credit score filled by income range and faceted over loan status.  These plots will be similar to the previous visual.  

# Construct similar scatter plots to the transparent, jittered scatter plots in the previous R chunk faceted over loan status (Reference 45). I had to add a guides command so that I could change the transparency of the colors in the legend (Reference 46). Note again that I am only considering incomes with the condition that they are verifiable and are not NA's (prosp_income).

ggplot(aes(x = credit_score_range_lower, y = borrower_apr, 
           color = income_range), 
       data = subset(prosp_income, !is.na(credit_score_range_lower) &
                             !is.na(borrower_apr))) + 
        geom_jitter(alpha = 1/10, position = position_jitter(h = 0)) + 
...
Next, I will look at bar charts of loan status filled by prosper score and
faceted over prosper rating.  I will do this to see which combination of 
prosper score and rating produces the best and worst outcomes of loan status.
I am doing this because the bar chart of the best and worst combination in the univariate analysis and the filled bar charts in the bivariate analysis seemed to indicate that there was a relationship between these variables.

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 8, fig.width = 11, loan_status_prosper_score_prosper_rating_graph}
# Plotting a bar graph of loan_status faceted by prosper rating (Referenc 43); proportions for each category filled by the proportion of each bar under each prosper score category (References 15 and 16).  I am using the dataframe for data post July 2009.

# I am using scale_fill_brewer to get a better color scheme (Reference 41)

ggplot(prosp_2009, aes(loan_status, fill = prosper_score)) + 
  geom_bar(aes(y=..count../sum(..count..))) + ylab('Proportion of Borrowers') +
  scale_fill_brewer(palette = "BrBG") + facet_wrap(~prosper_rating, ncol=2)  
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 5, fig.width = 10,Plot_One}

# This chunk represents a recap of the debt to income ratio versus credit score range lower second plot I presented in the multivariate analysis filled by income range.    

# To recap, this is a modified version of the second scatterplot in the group of 4 plots I made in the bivariate analysis representing debt to income ratio versus credit score range lower.  I have jittered the points and made them transparent while filling them with income range.  I did this so that I could see which portion of the plot is associated with each income range.  
...
# Graph scatter plot where the data is jittered and with a transparency of
# 1/10 to correct for overplotting (Reference 32).  I also cut off the data at debt to income ratio of 10 (Reference 21). They are outliers even though the data is good.  I set position_jitter with an h of 0 to keep the data in the vertical direction from becoming negative (Reference 33).  I am using scale_color_brewer to have a better color scheme (Reference 43).  
# For this final plot, I added a title, x axis label, y axis label and I centered the plot title (References 47 and 48).  I had to add a guides command so that I could change the transparency of the colors in the legend (Reference 46).  I used theme and theme_dark to change the background so that the colors would stand out better and to change the size of the axis labels (Reference 48 and 50).     
 ...
 ggplot(aes(x = credit_score_range_lower, y = debt_to_income_ratio, 
       ...
       data = subset(prosp_income, !is.na(credit_score_range_lower) &
        ...
        geom_jitter(alpha = 1/10,position = position_jitter(h = 0)) + 
        coord_cartesian(ylim = y_lim) +
        scale_color_brewer(name = "Income Range", palette = "BrBG") +
  ggtitle("Debt to Income Ratio vs Credit Score Range Lower by Income Range") + 
      xlab("Credit Score Range Lower") + ylab("Debt to Income Ratio") +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 7, fig.width = 10,Plot_Two}

# This chunk represents a recap of the Credit Score Range Lower versus Loan Status box plot I presented in the bivariate analysis.  This plot shows the median and mean of Credit Score Range Lower for each Loan Status to see how credit scores change for each group of loan status.  

# Find box plot for non-NA credit scores. I also zoomed in on the y axis to better see the medians and where they are (References 38 and 39).  For this final plot, I added a title, x axis label and y axis label (References 47).  I am using scale_fill_brewer to have a better color scheme (Reference 49).  I used theme and theme_dark to change the background so that the colors would stand out better and to center the title and change the size of the axis labels (Reference 48 and 50).  
ggplot(subset(prosp_sub, !is.na(credit_score_range_lower)), 
       aes(x = loan_status, y = credit_score_range_lower, fill = loan_status)) +
                  geom_boxplot() + 
                  stat_summary(fun.y=mean, geom="point", color = "red",
                               shape = 4, size=3) + 
                  coord_cartesian(ylim = c(500,800)) +
                  ggtitle("Credit Score Range Lower vs Loan Status") + 
        xlab("Loan Status") + ylab("Credit Score Range Lower") +  
                  scale_fill_brewer(name = "Loan Status", 
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height = 8, fig.width = 11, Plot_Three}
# This plot represents a recap of the bar chart of loan status filled by prosper score faceted over each prosper rating.  

# I am using scale_fill_brewer to get a better color scheme (Reference 43).  For this final plot, I added a title, x axis label and y axis label (Reference 47).  I used theme and theme_dark to change the background so that the colors would stand out better and to center the title and change the size of the axis labels (Reference 48 and 50).  I also used theme to set the size of each label for each bar chart.  

ggplot(prosp_2009, aes(loan_status, fill = prosper_score)) + 
  geom_bar(aes(y=..count../sum(..count..))) + 
  ylab('Proportion of Total Borrowers') + xlab('Loan Status') + 
  ggtitle('Proportion of Total Borrowers vs Loan Status') +
  scale_fill_brewer(name = "Prosper Score", palette = "BrBG") +
  facet_wrap(~prosper_rating, ncol=2)  +
  theme_dark() + theme(plot.title = element_text(hjust=0.5), 
                       panel.grid.major = element_line(color = "white"),
                        panel.grid.minor = element_line(color = "white"),
...
# Reference 4: https://stackoverflow.com/questions/29189663/trying-to-publish-an-r-notebook-and-keep-getting-the-same-error-error-in-contri

# Reference 5: http://kbroman.org/knitr_knutshell/pages/Rmarkdown.html

# Reference 6: https://stackoverflow.com/questions/5595512/what-is-the-difference-between-require-and-library

# Reference 7: http://rfunction.com/archives/1001

# Reference 8: https://stackoverflow.com/questions/6329962/split-code-over-multiple-lines-in-an-r-script
...
# Reference 11: https://www.rdocumentation.org/packages/utils/versions/3.4.3/topics/str

# Reference 12: https://stackoverflow.com/questions/11488820/why-use-c-to-define-vector

# Reference 13: https://www.rdocumentation.org/packages/base/versions/3.5.0/topics/subset

# Reference 14: http://www.sthda.com/english/wiki/renaming-data-frame-columns-in-r

# Reference 15: https://stats.idre.ucla.edu/r/modules/factor-variables/

# Reference 16: https://stackoverflow.com/questions/32845303/using-logical-operators-with-characters

# Reference 17: https://stackoverflow.com/questions/36604127/creating-a-bar-plot-with-proportions-on-ggplot

# Reference 18: http://ggplot2.tidyverse.org/reference/geom_bar.html

# Reference 19: http://tallman-world.tumblr.com/post/89964965867/rhow-to-rotate-axis-labels-in-ggplot2
...
# Reference 22: https://www.rdocumentation.org/packages/gridExtra/versions/2.3/topics/arrangeGrob

# Reference 23: https://stackoverflow.com/questions/2667673/select-first-4-rows-of-a-data-frame-in-r

# Reference 24: https://www.rdocumentation.org/packages/base/versions/3.5.0/topics/NA
...
# Reference 27: https://stackoverflow.com/questions/14708095/log10-transformation-in-r

# Reference 28: https://www.rdocumentation.org/packages/GGally/versions/1.3.2/topics/ggpairs

# Reference 29: https://stackoverflow.com/questions/5234117/how-to-drop-columns-by-name-in-a-data-frame
...
# Reference 41: http://ggplot2.tidyverse.org/reference/scale_continuous.html

# Reference 42: https://cran.r-project.org/web/packages/lemon/index.html

# Reference 43: http://www.sthda.com/english/wiki/colors-in-r

# Reference 44: https://cran.r-project.org/web/packages/lemon/vignettes/legends.html#shared-legend-across-multiple-plot

# Reference 45: https://plot.ly/ggplot2/facet_wrap/

# Reference 46: http://ggplot2.tidyverse.org/reference/guide_legend.html

# Reference 47: http://ggplot2.tidyverse.org/reference/labs.html

# Reference 48: https://stackoverflow.com/questions/40675778/center-plot-title-in-ggplot2

# Reference 49: http://t-redactyl.io/blog/2016/04/creating-plots-in-r-using-ggplot2-part-10-boxplots.html

# Reference 50:  http://ggplot2.tidyverse.org/reference/ggtheme.html

# Reference 51: https://stackoverflow.com/questions/13090838/r-markdown-avoiding-package-loading-messages/13090911
