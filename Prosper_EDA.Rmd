...
I considered, the largest porportions are for $25,000-49,999 and $50,000-74,999.
...
income of 'Not employed'.  It's hard to say again which of these variables
may be associated with higher levels of success or failure in paying back the
loans.  It would be assummed that the higher a borrower's income, the easier
it will be for them to pay back a loan but this isn't the only variable
affecting success or failure so I will need to further look at this in the 
bi-variate and multi-variate case.
...
outliers for those not employed of multiple years.  I cannot see Prosper letting 
a borrower have a loan who had not been employed for many years (I originally 
...
case of 743 years, not shown), I actually do think this data is possible and
that the information wasn't mis-entered.  Still though, I will perform some
operation to cut off the highest employment durations in the bi-variate and
multi-variate cases.
...
My dataset had 81 variables with 113,937 observations.  I only decided
to consider 9 total variables including loan_status, employment_status,
credit_score_range_lower, borrower_apr, debt_to_income_ratio, prosper_score,
prosper_rating, days_delinquent and is_homeowner.

The variables loan_status, employment_status, prosper_score, prosper_rating
and is_homeowner are all factor variables (note that I turned prosper_score
into a factor variable from a number variable) with the following levels:
...
Most of the borrowers are either in the 1-3 or 9-11 range of prosper scores
but if all the scores are considered separately, most of the borrowers have
either a 4 or 6 rating.  Most of the borrowers have a loan status of C.  And
around an equal number of borrowers are homeowners.
...
borrower should be trusted.  I also think borrower apr, credit score and debt-
to-income ratio will be beneficial to my investigation.  I may eliminate one of credit score or debt-to-income ratio if I think they are correlated with each other (my guess is that they are after looking at the two distributions).  I 
will look at this in the bi-variate analysis.  I wanted to see if employment status would support my investigation but the categories seem rather vague and 
are probably not useful at predicting loan payback success or failure.

### Unusual observations from the Univariate Analysis
